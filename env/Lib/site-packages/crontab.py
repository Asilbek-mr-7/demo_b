...
SYSTEMV = not WINOS and (
    os.uname()[0] in ["SunOS", "AIX", "HP-UX"] or os.uname()[4] in ["mips"]
)
...
class CronTab:
    ...
    def read(self, filename=None):
        ...
        if self.intab is not None:
            ...
            (out, err) = open_pipe(
                self.cron_command, l="", **self.user_opt
            ).communicate()
    ...
    def remove(self, *items):
        ...
        for item in items:
            if isinstance(item, (list, tuple, types.GeneratorType)):
                ...
                raise TypeError(
                    "You may only remove CronItem objects, "
...
class CronItem:
    ...
    def render(self, specials=True):
        ...
        if not self.is_valid() and self.enabled:
            raise ValueError(
                "Refusing to render invalid crontab." " Disable to continue."
            )
        ...
        if self.cron and self.cron.user is False:
            ...
            user = self.user + " "
        result = "%s %s%s" % (self.slices.render(specials=specials), user, command)
        if self.stdin:
    ...
    def schedule(self, date_from=None):
        ...
        try:
            ...
            raise ImportError(
                "Croniter not available. Please install croniter"
    ...
    def description(self, **kw):
        ...
        **kw - Keyword arguments to pass to cron_descriptor (see docs)
        ...
        try:
            ...
            raise ImportError(
                "cron_descriptor not available. Please install"
...
class CronSlices(list):
    ...
    def render(self, specials=True):
        ...
        if self.special and specials is not False:
            if self.special == "@reboot" or SPECIALS[self.special.strip("@")] == slices:
                return self.special
...
class CronSlice:
    ...
    def parse_value(self, val, sunday=None):
        ...
        if int(out) < self.min or int(out) > self.max:
            raise ValueError(
                "'{1}', not in {0.min}-{0.max} for {0.name}".format(self, val)
            )
        return out
...
def _render(value, resolve=False):
    ...
    return str("{:02d}".format(value) if ZERO_PAD else value)
