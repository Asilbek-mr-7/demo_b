...
class TimeZoneField(models.Field):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.use_pytz = (
            self.use_pytz_explicit
            if self.use_pytz_explicit is not None
            else use_pytz_default()
        )
        self.default_tzs = (
            self.default_pytz_tzs if self.use_pytz else self.default_zoneinfo_tzs
        )
        ...
        if self.choices_display == "WITH_GMT_OFFSET":
            ...
            raise ValueError(
                f"Unrecognized value for kwarg 'choices_display' of '{self.choices_display}'"
            )
    ...
    def validate(self, value, model_instance):
        if (self.use_pytz and not is_pytz_instance(value)) or (
            not self.use_pytz and not isinstance(value, ZoneInfo)
        ):
