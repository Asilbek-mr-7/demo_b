...
def generate_oid(node_id, process_id, thread_id, instance):
    ent = "{:x}-{:x}-{:x}-{:x}".format(node_id, process_id, thread_id, id(instance))
    try:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            exchange=(
                exchange if exchange is not None else Exchange(name, type="fanout")
            ),
            **kwargs,
        )
...
def insured(pool, fun, args, kwargs, errback=None, on_revive=None, **opts):
    ...
    with pool.acquire(block=True) as conn:
        ...
        insured = conn.autoretry(
            fun, channel, errback=errback, on_revive=revive, **opts
        )
