...
class AsyncAWSQueryConnection(AsyncConnection):
    ...
    def __init__(
        self, sqs_connection, http_client=None, http_client_params=None, **kwargs
    ):
    ...
    def get_object(
        self, operation, params, path="/", parent=None, verb="GET", callback=None
    ):  # noqa
        return self.make_request(
            operation,
            params,
            path,
            verb,
            callback=transform(self._on_obj_ready, callback, parent or self, operation),
        )
    ...
    def _on_list_ready(self, parent, markers, operation, response):
        ...
        if response.status == self.STATUS_CODE_OK:
            ...
            response.status in self.STATUS_CODES_TIMEOUT
            or response.status in self.STATUS_CODES_SERVER_ERRORS
        ):
