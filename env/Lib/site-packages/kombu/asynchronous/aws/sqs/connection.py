...
class AsyncSQSConnection(AsyncAWSQueryConnection):
    ...
    def __init__(self, sqs_connection, debug=0, region=None, **kwargs):
        ...
        super().__init__(sqs_connection, region_name=region, debug=debug, **kwargs)
    ...
    def create_queue(self, queue_name, visibility_timeout=None, callback=None):
        ...
        return self.get_object("CreateQueue", params, callback=callback)
    ...
    def send_message(self, queue, message_content, delay_seconds=None, callback=None):
    ...
    def change_message_visibility(
        self, queue, receipt_handle, visibility_timeout, callback=None
    ):
        return self.get_status(
            "ChangeMessageVisibility",
            {"ReceiptHandle": receipt_handle, "VisibilityTimeout": visibility_timeout},
            queue.id,
            callback=callback,
        )
    ...
    def _on_queue_ready(self, name, queues):
        return next(
            (q for q in queues if q.url.endswith(name)),
            None,
        )
    ...
    def add_permission(self, queue, label, aws_account_id, action_name, callback=None):
        return self.get_status(
            "AddPermission",
            {"Label": label, "AWSAccountId": aws_account_id, "ActionName": action_name},
            queue.id,
            callback=callback,
        )
