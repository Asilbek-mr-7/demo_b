...
class Exchange(MaybeChannelBound):
    ...
    def _can_declare(self):
        return not self.no_declare and (
            self.name and not self.name.startswith(INTERNAL_EXCHANGE_PREFIX)
        )
    ...
    def unbind_from(
        self, source="", routing_key="", nowait=False, arguments=None, channel=None
    ):
    ...
    def Message(self, body, delivery_mode=None, properties=None, **kwargs):
        ...
        if (isinstance(body, str) and properties.get("content_encoding", None)) is None:
            kwargs["content_encoding"] = "utf-8"
        return self.channel.prepare_message(body, properties=properties, **kwargs)

    def publish(
        self, message, routing_key=None, mandatory=False, immediate=False, exchange=None
    ):
        """Publish message.
    ...
    def delete(self, if_unused=False, nowait=False):
        ...
        Arguments:
            ...
            nowait (bool): If set the server will not respond, and a
                response will not be waited for. Default is :const:`False`.
        """
        return self.channel.exchange_delete(
...
class binding(Object):
    ...
    def __init__(
        self, exchange=None, routing_key="", arguments=None, unbind_arguments=None
    ):
    ...
    def __str__(self):
        return "{}->{}".format(
            _reprstr(self.exchange.name),
            _reprstr(self.routing_key),
        )
...
class Queue(MaybeChannelBound):
    ...
    def bind_to(
        self, exchange="", routing_key="", arguments=None, nowait=False, channel=None
    ):
        if isinstance(exchange, Exchange):
            exchange = exchange.name
    ...
    def queue_unbind(self, arguments=None, nowait=False, channel=None):
        return self.unbind_from(
            self.exchange, self.routing_key, arguments, nowait, channel
        )
    ...
    def __eq__(self, other):
        if isinstance(other, Queue):
            return (
                ...
                and self.queue_arguments == other.queue_arguments
                and self.binding_arguments == other.binding_arguments
                and self.consumer_arguments == other.consumer_arguments
                and self.durable == other.durable
