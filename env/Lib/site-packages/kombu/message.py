...
class Message:
    ...
    def ack_log_error(self, logger, errors, multiple=False):
        try:
            ...
            logger.critical(
                "Couldn't ack %r, reason:%r", self.delivery_tag, exc, exc_info=True
            )
            raise
    ...
    def reject_log_error(self, logger, errors, requeue=False):
        try:
            ...
            logger.critical(
                "Couldn't reject %r, reason: %r", self.delivery_tag, exc, exc_info=True
            )
    ...
    def _decode(self):
        return loads(
            self.body, self.content_type, self.content_encoding, accept=self.accept
        )
