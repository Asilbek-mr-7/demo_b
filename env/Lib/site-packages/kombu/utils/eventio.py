...
class _kqueue:
    w_fflags = KQ_NOTE_WRITE | KQ_NOTE_EXTEND | KQ_NOTE_ATTRIB | KQ_NOTE_DELETE
    ...
    def unwatch_file(self, fd):
        ev = kevent(
            fd, filter=KQ_FILTER_VNODE, flags=KQ_EV_DELETE, fflags=self.w_fflags
        )
        self._kcontrol([ev], 0)
    ...
    def _control(self, fd, events, flags):
        ...
        kevents = []
        if events & WRITE:
            kevents.append(kevent(fd, filter=KQ_FILTER_WRITE, flags=flags))
