...
class Connection:
        ...
        **kwargs,
        ...
        if hostname:
            ...
            if "://" in hostname and "+" in hostname[: hostname.index("://")]:
                ...
                params["transport"], params["hostname"] = hostname.split("+", 1)
        ...
        self.failover_strategy = (
            self.failover_strategies.get(self._failover_strategy)
            or self._failover_strategy
        )
        if self.alt:
    ...
    def _debug(self, msg, *args, **kwargs):
        if self._logger:  # pragma: no cover
            ...
            logger.debug(fmt.format(id=id(self), msg=str(msg)), *args, **kwargs)

    def connect(self):
        """Establish connection to server immediately."""
        return self._ensure_connection(max_retries=1, reraise_as_library_errors=False)
    ...
    def channel(self):
        ...
        if _log_channel:  # pragma: no cover
            ...
            return Logwrapped(chan, "kombu.channel", "[Kombu channel:{0.channel_id}] ")
        return chan
    ...
    def _info(self, resolve=True):
        transport_cls = self.transport_cls
        if resolve:
            transport_cls = self.resolve_aliases.get(transport_cls, transport_cls)
    ...
    def as_uri(
        ...
        mask="**",
        getfields=itemgetter("port", "userid", "password", "virtual_host", "transport"),
    ) -> str:
