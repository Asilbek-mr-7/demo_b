...
class ExchangeType:
    ...
    def equivalent(self, prev, exchange, type, durable, auto_delete, arguments):
        ...
        return (
            type == prev["type"]
            and durable == prev["durable"]
            and auto_delete == prev["auto_delete"]
            and (arguments or {}) == (prev["arguments"] or {})
        )
...
class TopicExchange(ExchangeType):
    ...
    def lookup(self, table, exchange, routing_key, default):
        return {
            queue for rkey, pattern, queue in table if self._match(pattern, routing_key)
        }
    ...
    def deliver(self, message, exchange, routing_key, **kwargs):
        ...
        for queue in [
            q for q in _lookup(exchange, routing_key) if q and q != deadletter
        ]:
            _put(queue, message, **kwargs)
