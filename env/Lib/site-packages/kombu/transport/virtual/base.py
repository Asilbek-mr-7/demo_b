...
class Message(base.Message):
    ...
    def serializable(self):
        ...
        body, _ = self.channel.encode_body(self.body, props.get("body_encoding"))
...
class Channel(AbstractChannel, base.StdChannel):
    ...
    def exchange_declare(
        ...
        try:
            ...
            if not self.typeof(exchange).equivalent(
                prev, exchange, type, durable, auto_delete, arguments
            ):
                raise NotEquivalentError(
                    NOT_EQUIVALENT_FMT.format(
                        exchange, self.connection.client.virtual_host or "/"
                    )
                )
        except KeyError:
    ...
    def exchange_bind(
        self, destination, source="", routing_key="", nowait=False, arguments=None
    ):
    ...
    def queue_bind(
        self, queue, exchange=None, routing_key="", arguments=None, **kwargs
    ):
        ...
        if not R and default is not None:
            warnings.warn(
                UndeliverableWarning(
                    UNDELIVERABLE_FMT.format(exchange=exchange, routing_key=routing_key)
                ),
            )
    ...
    def _reset_cycle(self):
        self._cycle = FairCycle(self._get_and_deliver, self._active_queues, Empty)
    ...
    def _get_message_priority(self, message, reverse=False):
        ...
        try:
            priority = max(
                min(int(message["properties"]["priority"]), self.max_priority),
                self.min_priority,
            )
    ...
    def _get_free_channel_id(self):
        ...
        raise ResourceError(
            "No free channel ids, current={}, channel_max={}".format(
                len(self.connection.channels), self.connection.channel_max
            ),
            (20, 10),
        )
...
class Management(base.Management):
    ...
    def get_bindings(self):
        return [
            {"destination": q, "source": e, "routing_key": r}
...
class Transport(base.Transport):
    ...
    def _deliver(self, message, queue):
        if not queue:
            raise KeyError(
                "Received message without destination queue: {}".format(message)
            )
        try:
    ...
    def on_message_ready(self, channel, message, queue):
        if not queue or queue not in self._callbacks:
            raise KeyError(
                "Message for queue {!r} without consumers: {}".format(queue, message)
            )
