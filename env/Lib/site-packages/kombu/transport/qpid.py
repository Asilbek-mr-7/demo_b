...
class QoS:
    ...
    def can_consume_max_estimate(self):
        ...
        return (
            ...
            else (self.prefetch_count - len(self._not_yet_acked))
        )
...
class Channel(base.StdChannel):
    ...
    def _put(self, routing_key, message, exchange=None, durable=True, **kwargs):
        ...
        """
        if not exchange:
            address = f"{routing_key}; " "{{assert: always, node: {{type: queue}}}}"
            msg_subject = None
        else:
            address = (
        ...
        qpid_message = qpid.messaging.Message(
            content=message, durable=durable, subject=msg_subject
        )
        try:
    ...
    def queue_declare(
        ...
        return amqp.protocol.queue_declare_ok_t(queue, message_count, consumer_count)
...
class Connection:
    ...
    def __init__(self, **connection_options):
        ...
        try:
            msg = (
                _("Attempting to connect to qpid with " "SASL mechanism %s") % sasl_mech
            )
            logger.debug(msg)
            self._qpid_conn = establish(**self.connection_options)
            # connection was successful if we got this far
            msg = _("Connected to qpid with SASL " "mechanism %s") % sasl_mech
            logger.info(msg)
            ...
            contains_mech_fail_text = (
                "sasl negotiation failed: no mechanism agreed" in conn_exc.text
            )
            contains_mech_unavail_text = "no mechanism available" in conn_exc.text
            if (
                coded_as_auth_failure
                or contains_auth_fail_text
                or contains_mech_fail_text
                or contains_mech_unavail_text
            ):
                msg = (
                    _("Unable to connect to qpid with SASL " "mechanism %s") % sasl_mech
                )
                logger.error(msg)
...
class Transport(base.Transport):
    ...
    recoverable_channel_errors = (NotFound,)
