...
class Channel(virtual.Channel):
    ...
    def _get_or_create_session(self, queue):
        ...
        try:
            ...
            session_id = None
        return (
            self._renew_existing_session(session_id)
            if session_id is not None
            else self._create_new_session()
        )
    ...
    def _create_new_session(self):
        logger.debug(
            "Creating session %s with TTL %s", self.lock_name, self.session_ttl
        )
    ...
    def _acquire_lock(self, queue, raising=LockError):
        ...
        logger.debug(
            "Trying to create lock object %s with session %s", lock_key, session_id
        )

        if self.client.kv.put(key=lock_key, acquire=session_id, value=self.lock_name):
    ...
    def _get(self, queue, timeout=None):
        ...
        with self._queue_lock(queue, raising=Empty):
            ...
            try:
                ...
                logger.debug(
                    "Removing key %s with modifyindex %s",
                    data[0]["Key"],
                    data[0]["ModifyIndex"],
                )

                self.client.kv.delete(key=data[0]["Key"], cas=data[0]["ModifyIndex"])
    ...
    def _size(self, queue):
        ...
        try:
            key = f"{self._key_prefix(queue)}/msg/"
            logger.debug("Fetching key recursively %s with index %s", key, self.index)
            self.index, data = self.client.kv.get(
                key=key,
                recurse=True,
                index=self.index,
        ...
        logger.debug("Found %s keys under %s with index %s", size, key, self.index)
        return size
