...
class Channel(virtual.Channel):
    ...
    def __init__(self, *args, **kwargs):
        if QueueService is None:
            raise ImportError(
                "Azure Storage Queues transport requires the "
    ...
    def _get(self, queue, timeout=None):
        ...
        messages = self.queue_service.get_messages(q, num_messages=1, timeout=timeout)
        if not messages:
            raise Empty()
    ...
    def queue_service(self):
        if self._queue_service is None:
            self._queue_service = QueueService(
                account_name=self.conninfo.hostname, account_key=self.conninfo.password
            )
