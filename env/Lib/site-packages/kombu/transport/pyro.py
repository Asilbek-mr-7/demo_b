...
class Transport(virtual.Transport):
    ...
    def _open(self):
        ...
        try:
            ...
            reraise(
                NamingError,
                NamingError(E_NAMESERVER.format(conninfo)),
                sys.exc_info()[2],
            )
        try:
            ...
            reraise(
                NamingError, NamingError(E_LOOKUP.format(conninfo)), sys.exc_info()[2]
            )
...
if __name__ == "__main__":
    ...
    with pyro.Daemon() as daemon:
        print(
            "(Expecting a Pyro name server at {}:{})".format(
                pyro.config.NS_HOST, pyro.config.NS_PORT
            )
        )
        with pyro.locateNS() as ns:
            print(
                "You can connect with Kombu using the url "
