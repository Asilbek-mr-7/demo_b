...
class Channel(virtual.Channel):
    ...
    def _get_or_create(self, queue):
        obj = (
            self.session.query(self.queue_cls)
            .filter(self.queue_cls.name == queue)
            .first()
        )
        if not obj:
            with _MUTEX:
    ...
    def _query_all(self, queue):
        obj = self._get_or_create(queue)
        return self.session.query(self.message_cls).filter(
            self.message_cls.queue_id == obj.id
        )
    ...
    def queue_cls(self):
        return self._declarative_cls(
            "Queue", QueueBase, {"__tablename__": self.queue_tablename}
        )
