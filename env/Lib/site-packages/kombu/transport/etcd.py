...
class Channel(virtual.Channel):
    ...
    def _put(self, queue, payload, **_):
        ...
        with self._queue_lock(queue):
            ...
            if not self.client.write(key=key, value=dumps(payload), append=True):
        ...
        with self._queue_lock(queue):
            ...
            try:
                result = self.client.read(
                    key=key, recursive=True, index=self.index, timeout=self.timeout
                )
    ...
    def _size(self, queue):
        ...
        with self._queue_lock(queue):
            ...
            try:
                key = self._key_prefix(queue)
                logger.debug(
                    "Fetching key recursively %s with index %s", key, self.index
                )
                result = self.client.read(key=key, recursive=True, index=self.index)
...
class Transport(virtual.Transport):
    ...
    if etcd:
        connection_errors = virtual.Transport.connection_errors + (etcd.EtcdException,)

        channel_errors = virtual.Transport.channel_errors + (etcd.EtcdException,)
