...
class MaybeEncodingError(Exception):
    ...
    def __str__(self):
        return "Error sending result: '%r'. Reason: '%r'." % (self.value, self.exc)
                ...
                if req:
                    ...
                    if max_memory_per_child > 0:
                        ...
                        if used_kb > 0 and used_kb > max_memory_per_child:
                            warning(
                                MAXMEM_USED_FMT.format(used_kb, max_memory_per_child)
                            )
                            return EX_RECYCLE
...
class TimeoutHandler(PoolThread):
    ...
    def _process_by_pid(self, pid):
        return next(
            ((proc, i) for i, proc in enumerate(self.processes) if proc.pid == pid),
            (None, None),
        )
    ...
    def _trywaitkill(self, worker):
        ...
        try:
            if os.getpgid(worker.pid) == worker.pid:
                debug(
                    "worker %s is a group leader. It is safe to kill (SIGTERM) the whole group",
                    worker.pid,
                )
        ...
        try:
            if os.getpgid(worker.pid) == worker.pid:
                debug(
                    "worker %s is a group leader. It is safe to kill (SIGKILL) the whole group",
                    worker.pid,
                )
            ...
            try:
                ...
                if not time_terminate:
                    ...
                    if now - time_terminate > 5.0:
                        ...
                        break
                    debug(
                        "result handler: all workers terminated, " "timeout in %ss",
...
class Pool(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enable_timeouts = bool(
            enable_timeouts or self.timeout is not None or self.soft_timeout is not None
        )
        ...
        self._task_handler = self.TaskHandler(
            self._taskqueue, self._quick_put, self._outqueue, self._pool, self._cache
        )
        if threads:
    ...
    def create_result_handler(self, **extra_kwargs):
        return self.ResultHandler(
            ...
            self.check_timeouts,
            self.on_job_ready,
            on_ready_counters=self._on_ready_counters,
            **extra_kwargs
        )
    ...
    def _process_by_pid(self, pid):
        return next(
            ((proc, i) for i, proc in enumerate(self._pool) if proc.pid == pid),
            (None, None),
        )
    ...
    def _join_exited_workers(self, shutdown=False):
        ...
        for i in reversed(range(len(self._pool))):
            ...
            if popen is None or exitcode is not None:
                ...
                if exitcode not in (EX_OK, EX_RECYCLE) and not getattr(
                    worker, "_controlled_termination", False
                ):
                    error(
        ...
        if self._worker_handler._state == RUN and self._state == RUN:
            try:
                ...
                if get_errno(exc) == errno.ENOMEM:
                    reraise(MemoryError, MemoryError(str(exc)), sys.exc_info()[2])
                raise
    ...
    def imap(self, func, iterable, chunksize=1, lost_worker_timeout=None):
        ...
        if chunksize == 1:
            result = IMapIterator(self._cache, lost_worker_timeout=lost_worker_timeout)
            self._taskqueue.put(
                (
                    (
    ...
    def imap_unordered(self, func, iterable, chunksize=1, lost_worker_timeout=None):
        """
    ...
    def _set(self, i, obj):
        with self._mutex:
            ...
            if self._callback and self._success:
                self.safe_apply_callback(self._callback, self._value)
            if self._value is not None and self._error_callback and not self._success:
                self.safe_apply_callback(self._error_callback, self._value)
